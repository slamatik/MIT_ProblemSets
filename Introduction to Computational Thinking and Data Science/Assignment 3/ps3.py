...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        x = math.floor(pos.x)
        y = math.floor(pos.y)
        amount_of_dirt = self.room[(x, y)]
        if amount_of_dirt >= capacity:
...
class Robot(object):
    ...
    def __init__(self, room: RectangularRoom, speed, capacity):
        ...
        self.cleaning_capacity = capacity
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        xy = (math.floor(pos.x), math.floor(pos.y))
        return xy in self.furniture_tiles
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        next_position = current_position.get_new_position(self.robot_direction, self.robot_speed)
        if self.room.is_position_valid(next_position):
            self.robot_position = next_position
            self.room.clean_tile_at_position(self.robot_position, self.cleaning_capacity)
        else:
            self.robot_direction = random.randint(0, 360)
